...
gbts_seeding_algorithm::output_type gbts_seeding_algorithm::operator()(
    ...
    ctx.nMaxEdges = 10 * ctx.nNodes; // TODO: make this tuneable
    ...
    if (ctx.nEdges > ctx.nMaxEdges){
        TRACCC_ERROR("Removing " << ctx.nEdges - ctx.nMaxEdges << " Edges, over the cap of " << ctx.nMaxEdges);
    ...
    TRACCC_DEBUG("Proposed Seeds " << nProps);
    ...
    TRACCC_DEBUG("GBTS found " << m_copy.get().get_size(output_seeds) << " seeds"); // Never populated 
    return output_seeds;
}
