    ...
    if (spIdx >= nSp) {
        ...
        if (begin_or_bin < 0) {
            ...
            for (unsigned int surface = -1 * (begin_or_bin + 1);
                ...
                std::pair<unsigned int, unsigned int> surfaceBinPair =
...
__global__ void node_eta_binning_kernel(
    ...
    collection_types<int>::device layerCounts(layerCounts_view);
    const collection_types<float4>::const_device d_sp_params(d_sp_params_view);
    collection_types<std::pair<int, int>>::const_device d_layer_info(
        d_layer_info_view);
    collection_types<std::pair<float, float>>::const_device d_layer_geo(
        d_layer_geo_view);
...
__global__ void minmax_rad_kernel(
    const collection_types<int>::const_view
        d_eta_bin_views_view,  // Here the type changed as a quick fix to avoid
                               // the issue with the eta_bin_views buffer
    ...
    const collection_types<float>::const_device d_node_params(
        d_node_params_view);
    collection_types<float>::device d_bin_rads(
        d_bin_rads_view);  // Here the type changed as a quick fix to avoid the
                           // issue with the bin_rads buffer
    ...
    int node_start =
        d_eta_bin_views[2 * eta_bin_idx];  // Here the type changed as a quick
                                           // fix to avoid the issue with the
                                           // eta_bin_views buffer
    ...
    d_bin_rads[2 * eta_bin_idx] =
        min_r;  // Here the type changed as a quick fix to avoid the issue with
                // the bin_rads buffer
