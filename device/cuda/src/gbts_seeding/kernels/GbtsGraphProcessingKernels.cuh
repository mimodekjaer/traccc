 ...
 */
 ...
 inline TRACCC_DEVICE void add_seed_proposal(const int qual, const int path_idx,
     ...
     while (path.y >= 0 && depth != 0) {
         ...
         unsigned long long int old_val = atomic_bid.load(); // replace with fetch_max when in vecmem
...
void __global__ seeds_bid_for_hits(const collection_types<int>::const_view d_output_graph_view, 
    ...
    for (unsigned int prop_idx = threadIdx.x + blockDim.x * blockIdx.x;
        ...
        while (path.y >= 0) {
            ...
            int sp_idx = d_output_graph[traccc::device::gbts_consts::node1 +
                                        edge_size * path.x];
            vecmem::device_atomic_ref<unsigned long long int> atomic_bid(d_hit_bids[sp_idx]);
            unsigned long long int old_val = atomic_bid.load(); // replace with fetch_max when in vecmem updated
            while (seed_bid > old_val &&
                !atomic_bid.compare_exchange_strong(old_val, seed_bid)) {
                // old_val is updated by compare_exchange_strong on failure, loop retries
            }
        ...
        int sp_idx = d_output_graph[traccc::device::gbts_consts::node2 +
                                    edge_size * path.x];
        vecmem::device_atomic_ref<unsigned long long int> atomic_bid(d_hit_bids[sp_idx]);
        unsigned long long int old_val = atomic_bid.load(); // replace with fetch_max when in vecmem is updated
        while (seed_bid > old_val &&
            !atomic_bid.compare_exchange_strong(old_val, seed_bid)) {
            // old_val is updated by compare_exchange_strong on failure, loop retries
        }
