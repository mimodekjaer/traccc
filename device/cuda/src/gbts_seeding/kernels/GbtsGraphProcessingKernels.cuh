...
__global__ static void CCA_IterationKernel(
    ...
    const collection_types<unsigned int>::view d_counters_view, const int iter,
    ...
    if (threadIdx.x == 0) {
        if (vecmem::device_atomic_ref<unsigned int>(d_counters[5])
                .fetch_add(1) == gridDim.x - 1) {
...
void __global__
count_terminus_edges(const collection_types<short2>::view d_outgoing_paths_view,
    ...
    if (edge_idx < nEdges) {
        ...
        // paths
        if (out_paths.y != -1) {
            d_outgoing_paths[edge_idx].y =
                vecmem::device_atomic_ref<unsigned int>(d_counters[7])
                    .fetch_add(1);
    ...
    barrier().blockBarrier();
    if (threadIdx.x == 0) {
        vecmem::device_atomic_ref<unsigned int>(d_counters[6])
            .fetch_add(outgoingCount);
    }
}

void __global__ add_terminus_to_path_store(
    ...
    collection_types<short2>::const_device d_outgoing_paths(
...
// registers
void __global__ fill_path_store(
    const collection_types<int2>::view d_path_store_view,
    const collection_types<int>::const_view d_output_graph_view,
 ...
 */
inline TRACCC_DEVICE void add_seed_proposal(
    ...
    while (path.y >= 0 && depth != 0) {
        ...
        //     atomicMax(&d_edge_bids[path.x], seed_bid); /// TODO Change to
        //     device atomic max
        ...
        unsigned long long int old_val =
            atomic_bid.load();  // replace with fetch_max when in vecmem
        while (seed_bid > old_val &&
               !atomic_bid.compare_exchange_strong(old_val, seed_bid)) {
            // old_val is updated by compare_exchange_strong on failure, loop
            // retries
        }
...
void __global__ fit_segments(
    ...
    int prop_idx =
        vecmem::device_atomic_ref<unsigned int>(d_counters[8]).fetch_add(1);
...
void __global__ reset_edge_bids(
    const collection_types<int2>::const_view d_path_store_view,
    const collection_types<int2>::view d_seed_proposals_view,
    const collection_types<unsigned long long int>::view d_edge_bids_view,
    const collection_types<char>::view d_seed_ambiguity_view,
    ...
    for (int prop_idx = threadIdx.x + blockIdx.x * blockDim.x;
        ...
        if (round == -1) {
            if (ambi == 0) {
                ...
                vecmem::device_atomic_ref<unsigned int>(d_counters[9])
                    .fetch_add(1);
                continue;
            }
...
void __global__ seeds_bid_for_hits(
    const collection_types<int>::const_view d_output_graph_view,
    ...
    for (unsigned int prop_idx = threadIdx.x + blockDim.x * blockIdx.x;
        ...
        while (path.y >= 0) {
            ...
            unsigned long long int old_val =
                atomic_bid
                    .load();  // replace with fetch_max when in vecmem updated
            while (seed_bid > old_val &&
                   !atomic_bid.compare_exchange_strong(old_val, seed_bid)) {
                // old_val is updated by compare_exchange_strong on failure,
                // loop retries
            }
        }
        ...
        unsigned long long int old_val =
            atomic_bid
                .load();  // replace with fetch_max when in vecmem is updated
        while (seed_bid > old_val &&
               !atomic_bid.compare_exchange_strong(old_val, seed_bid)) {
            // old_val is updated by compare_exchange_strong on failure, loop
            // retries
...
inline __device__ float2 estimate_params(const std::array<float4, 3>& sps) {
...
void __global__ gbts_seed_conversion_kernel(
    ...
    for (int prop_idx = threadIdx.x + blockIdx.x * blockDim.x;
        ...
        if (use_dropout) {
            ...
            std::array<float, 3> diff = {abs(curv_cot_1.x - curv_cot_2.x),
