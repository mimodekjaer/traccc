...
TRACCC_HOST_DEVICE inline void graph_edge_matching_body(
    ...
    const collection_types<float>::const_device d_edge_exp_eta(
        d_edge_exp_eta_view);
    const collection_types<float>::const_device d_edge_curv(d_edge_curv_view);
    const collection_types<float>::const_device d_edge_phi_z(d_edge_phi_z_view);
    const collection_types<float>::const_device d_edge_phi_w(d_edge_phi_w_view);
    ...
    if (nLinks != 0u) {
        ...
        const float uat_2 = 1.0f / d_edge_exp_eta[globalIndex];
        const float curv2 = d_edge_curv[globalIndex];
        const float Phi2  = d_edge_phi_z[globalIndex];
        ...
        // for the one 4-byte exp_eta load.
        unsigned int next_edge2_idx = d_edge_links[link_begin];
        float next_exp_eta = d_edge_exp_eta[next_edge2_idx];
        ...
        for (unsigned int k = 0u; k < nLinks; ++k) {
            ...
            const float exp_eta_2 = next_exp_eta;
            ...
            const float curv_2 = d_edge_curv[edge2_idx];
            const float dcurv  = curv_2 - curv2;
            if (fabsf(dcurv) > cut_dcurv_max) {
                continue;
            }

            const float phi_w_2 = d_edge_phi_w[edge2_idx];
