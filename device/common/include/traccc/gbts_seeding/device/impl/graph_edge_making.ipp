...
TRACCC_HOST_DEVICE inline void graph_edge_making(
    ...
    const collection_types<float4>::const_device d_node_params(
        d_node_params_view);
    const collection_types<float>::const_device d_node_phi(d_node_phi_view);
    collection_types<uint2>::device d_edge_nodes(d_edge_nodes_view);
    collection_types<gbts_edge4>::device d_edge_params(d_edge_params_view);
        ...
        if (!boundary) {
            for (; n1Idx < num_nodes1; n1Idx++) {
                ...
                gbts_detail::gbts_checks(
                    ...
                    deltaPhi, d_gbts_edge_making_params, nMaxEdges);
            }
        } else {
            for (; n1Idx < num_nodes1; n1Idx++) {
                const float phi1 = phi[n1Idx];
