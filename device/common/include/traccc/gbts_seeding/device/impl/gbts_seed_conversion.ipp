...
TRACCC_HOST_DEVICE inline traccc::float2 estimate_params(
    ...
    const float x0 = sps[1][0];
    const float y0 = sps[1][1];
    ...
    for (unsigned int k = 0; k < 2; k++) {
        ...
        const float dx = sps[sp_idx][0] - x0;
        const float dy = sps[sp_idx][1] - y0;
    ...
    const float cot_t = (sps[2][2] - sps[1][2]) /
                        (sqrtf(sps[2][0] * sps[2][0] + sps[2][1] * sps[2][1]) - r0);
    ...
    if (d_seed_ambiguity[prop_idx] == -2) {
        ...
        if (use_dropout) {
            ...
            if ((best_for_hit < seed.size - 1) &
                (fabsf(curv_cot_1[1] + curv_cot_2[1] + curv_cot_3[1]) <
                 3.0f * tight_bid_cot_threshold) &
                (seed.size < 5)) {
                return;
            }
            std::array<float, 3> diff = {fabsf(curv_cot_1[0] - curv_cot_2[0]),
                                         fabsf(curv_cot_2[0] - curv_cot_3[0]),
                                         fabsf(curv_cot_1[0] - curv_cot_3[0])};
            diff_code = static_cast<char>(4 * (diff[0] < dcurv_cut_m) +
                                          2 * (diff[1] < dcurv_cut_m) +
                                          (diff[2] < dcurv_cut_m));
            force_dropout = fabsf(curv_cot_1[0] + curv_cot_2[0] + curv_cot_3[0]) <
        ...
        float quality = static_cast<float>(prop[0]);
